...
def print_event(event,indent,alreadySeen,lines,neededLabels,noPrintNxt=False,isAnd=False):
    ...
    if isinstance(event.data, ActionEvent):
        lines.append(spaces+str(event.data.actor.v.identifier)+"."+str(event.data.actor_action.v)+"("+str(event.data.params.data if event.data.params else '')+")\n")
        if not noPrintNxt:
...
if __name__ == "__main__":
    for fname in glob.glob("event/**/*.bfevfl", recursive=True):
        #print(fname)
        ...
        try:
            ...
            print(fname, 'cannot be processed - AssertionError:', traceback.format_exc().split("\n")[-3].strip())
            continue

        flowchart = flow.flowchart
        fname2 = fname.replace("event"+os.sep,"event_txt"+os.sep).replace(".bfevfl",".c")
        assert fname != fname2
        try:
            os.makedirs(fname2.rsplit(os.sep,1)[0])
        except OSError as e:
            pass
        outfile = open(fname2,'w',encoding='utf-8')
