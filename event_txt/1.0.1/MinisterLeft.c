...
void MinisterTalk() {
    ...
    TransitCounters.SetValue({'key': 'MinisterLeftLeadingStart', 'value': 1})
...
void LeadingRestart() {
    Fade.StartPreset({'preset': 'FadeOutFastB'})
    MinisterLeft.WarpToLinkedPoint({'index': 0, 'offsetY': 0.0})
    MinisterLeft.SetInstanceVariableInt({'name': 'pathIndex', 'value': 0})
    MinisterLeft.AimCompassPoint({'direction': 0, 'immediateTurn': True, 'duration': 0.0, 'withoutTurn': False})
    Fade.StartPreset({'preset': 'FadeInFast'})
}
