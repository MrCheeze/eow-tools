...
Actor: Zelda
entrypoint: None()
actions: ['AimActor', 'AimCompassPoint', 'WarpToActor', 'SetInterestIkEnabled', 'DestroyCollisionHitActors', 'SetGravityEnable', 'PlayAnimationNoWait']
...
Actor: GameControl
entrypoint: None()
actions: ['UnforbidSave', 'EnableAutoSave', 'ForbidWarp', 'RequestAutoSave', 'UnforbidWarp', 'SetResetOverlappedActorsOnMoveDone']
queries: []
params: None
...
void PleaseCopySword_S() {
    ...
    fork {
        ...
        Zelda.WarpToActor({'offsetY': 0.0, 'distance': 0.0, 'actor': ActorIdentifier(name="GatePartnerPhoton"), 'offsetX': 0.0})
        Timer.Wait({'time': 0.10000000149011612})
        Zelda.AimCompassPoint({'direction': 2, 'immediateTurn': True, 'duration': 0.4000000059604645, 'withoutTurn': False})
...
void PleaseLookLinkBarricade() {
    ...
    fork {
        ...
        Zelda.SetInterestIkEnabled({'enable': False})
        GameControl.SetResetOverlappedActorsOnMoveDone({'enable': False})
        Zelda.WarpToActor({'offsetY': 0.0, 'actor': ActorIdentifier(name="GatePartnerPhoton"), 'distance': 0.0, 'offsetX': 0.0})
        Timer.Wait({'time': 0.10000000149011612})
        Zelda.AimCompassPoint({'direction': 2, 'immediateTurn': True, 'duration': 0.4000000059604645, 'withoutTurn': False})
        Partner.WarpToDefaultPosition({'position': 2, 'avoidWall': False, 'avoidPlayer': False})
        Zelda.DestroyCollisionHitActors({'eventReset': True})
